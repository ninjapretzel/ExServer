...
## ============>
### 2019/07/28
Been a while, had life stuff come up and couldn't pour my head into this project. I really want to come back to this and hammer it out by end of year.
I haven't been entirely unproductive, but haven't touched this codebase in months.
It is a good idea to go over some of what I have done outside of here that pertains to things I want to accomplish.

First, I have had some ideas for utilizing compute shaders in the server for server-side generation and verification of stuff, such as collision solving and terrain generation.
I wrote a number of compute shaders inside of unity for generating terrains for planets, that was fairly easy. I even augmented them with techniques from "No Man's Sky" such as "Analytical Derivitave Noise".
These, I would like to use on the server to verify collisions with terrain by height. 
I'd ideally like to re-use the code in such a way that I can maintain one shader codebase, and use it wherever needed (Software in C#, or hardware on the server/client as needed), howver that is prohibitave.
Transpiling could be an option, but it requires a lot of knowledge that I don't have to do gracefully. The solutions I imagine right now are very hacky and prone to breaking at slight changes.
I could just nix the whole idea, but it would be very valuable to know the heights of terrains at any positions on the server, to verify terrain collisions for players and have mobs placed properly by the server itself.
Instead of me pouring as much energy into solving that problem as I was, I'm finally cutting it off and deciding to do it later.

Second, I have been working on a scripting language using my Json library as a basis. It's fairly early in development, but has proven a useful learning experience.
It could be useful to integrate it somewhere into the server at some point, but that will be further off in the future.

Third, I have been researching using neural networks to generate content, one of the plans for this game is to use generated content in various places.
I don't know what the infrastructure would be, but I could see a webserver holding and serving the generated content, and placeholder content being used client-side until it finishes generating and is recieved/imported.
There has been some success in various fields, such as generating photos of faces, or even illustrations. 
It would be exciting to have other content generated by neural networks, such as sound effects or models.

We'll have to see how feasable it is to generate such things when it comes time to integrate them.
