...
namespace Poly {
	...
	public class StatCalc : DBData {
		...
		/// <summary> Fully recalculate a unit's stats. Assumed all preprocessing (eg equipment and skills) is applied. </summary>
		/// <param name="unit"> Unit data to recalc for. </param>
		public void FullRecalc(UnitStats unit) {
			...
			data.Set(unit.baseStats);

			JsonObject result = CalcStats(data);
		...
		public JsonObject CalcStats(JsonObject fullData) {
			//Log.Info($"FullData is {fullData}");
			//Log.Info($"DB BaseStats is {BaseStats}");

			JsonObject baseStats = SmartMask(fullData, BaseStats);
			//Log.Info($"masked baseStats is {baseStats}");
		...
		public JsonObject MatrixMultiply(JsonObject lhs, JsonObject rhs) {
				...
				foreach (var pair in row) {
					...
					if (pair.Value.isNumber) {
						...
						if (key.StartsWith("@")) { // Anchor
							...
							//Log.Info($"StatCalc.MatrixMult.MultiplyRow: Multiplying data[{key}]={lhs.GetNumber(pair.Key)} by {pair.Value}");
			...
			foreach (var pair in rhs) {
				...
				//Log.Info($"StatCalc.MatrixMult: Multiplying Row for {key}, {val}");
