...
namespace Ex {
	...
	public static class Request {
		...
		/// <param name="callback"> Callback to fire on success </param>
		/// <returns> Task that can be awaited for the completion. </returns>
		public static Task<byte[]> GetRaw(string url, Action<byte[]> callback) {
		...
		/// <param name="callback"> Callback to fire on success </param>
		/// <returns> Awaitable <see cref="Task"/> </returns>
		public static async Task<byte[]> GetRawAsync(string url, Action<byte[]> callback) {
			try {
				HttpResponseMessage response = await http.GetAsync(url);
		...
		public static Task<string> Post(string url, string content, Action<string> callback, Func<string, HttpContent> prep = null) {
		...
		/// <returns> Awaitable Task </returns>
		/// <remarks> Invokes <see cref="onError"/> during any failures. </remarks>
		public static async Task<string> PostAsync(string url, string content, Action<string> callback, Func<string, HttpContent> prep = null) {
			try {
				HttpContent request;
